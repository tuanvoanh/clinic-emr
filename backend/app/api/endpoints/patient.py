...
@router.get("/", response_model=List[PatientResponse], summary="Search patients by phone number prefix (min 1 digit)", description="""
Search patients by phone prefix using index-friendly startswith (e.g. '8' or '812345...').
Accepts 1 to 8 digits and requires the prefix to start with 8 or 9.
...
def search_patients(
    phone: str = Query(
        ...
        max_length=8,
        pattern=r"^[89]\d{0,7}$",
        description="Singapore mobile phone number prefix to search (1 to 8 digits, starting with 8 or 9)"
    ),
